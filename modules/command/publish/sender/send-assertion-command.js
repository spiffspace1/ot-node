...
class SendAssertionCommand extends Command {
    ...
    async execute(command) {
        ...
        if (assertion.metadata.visibility === 'private') {
            nquads = nquads.filter((x) => x.startsWith(`<${DID_PREFIX}:`));
        }

        let busyResponses = 0;
        let failedResponses = 0;
        const storePromises = nodes.map((node) =>
            this.publishService
                ...
                .then((response) => {
                    if (!response) {
                        this.logger.error({
                            ...
                            Event_name: ERROR_TYPE.SEND_ASSERTION_ERROR,
                            Id_operation: handlerId,
                        });
                        failedResponses += 1;
                    } else if (response === NETWORK_RESPONSES.BUSY) {
                        this.logger.error({
                            ...
                            Operation_name: 'Error',
                            Event_name: ERROR_TYPE.SEND_ASSERTION_ERROR_BUSY,
                            Id_operation: handlerId,
                        });
                        busyResponses += 1;
                    }
        ...
        const maxFailResponses = Math.round((1 - STORE_MIN_SUCCESS_RATE) * nodes.length);
