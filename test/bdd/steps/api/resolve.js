...
When(/^I call resolve on node (\d+) for last published assertion/, { timeout: 120000 }, async function (node) {
    ...
    expect(!!this.state.lastPublishData, 'Last publish data is undefined. Publish is not finalized.').to.be.equal(true);
    const assertionIds = [this.state.lastPublishData.result.data.id];
    const result = await this.state.nodes[node - 1].client
        .resolve(assertionIds).catch((error) => {
...
Given('I wait for last resolve to finalize', { timeout: 120000 }, async function () {
    ...
    expect(!!this.state.lastResolveData, 'Last resolve data is undefined. Resolve is not started.').to.be.equal(true);
    const resolveData = this.state.lastResolveData;
    let loopForResolveResult = true;
    let retryCount = 0;
    const maxRetryCount = 2;
    while (loopForResolveResult) {
        this.logger.log(`Getting resolve result for handler id: ${resolveData.handlerId} on node: ${resolveData.nodeId}`);
        // eslint-disable-next-line no-await-in-loop
        const resolveResult = await this.state.nodes[resolveData.nodeId].client.getResult(resolveData.handlerId, 'resolve').catch((error) => {
...
Given(/Last resolve finished with status: ([COMPLETED|FAILED]+)$/, { timeout: 120000 }, async function (status) {
    this.logger.log(`Last resolve finished with status: ${status}`);
    expect(!!this.state.lastResolveData, 'Last resolve data is undefined. Resolve is not started.').to.be.equal(true);
    expect(!!this.state.lastResolveData.result, 'Last resolve data result is undefined. Resolve is not finished.').to.be.equal(true);

    const resolveData = this.state.lastResolveData;
    expect(resolveData.result.status, 'Publish result status validation failed').to.be.equal(status);
});
