Feature: Searching for an assertions and entities on the DKG
  ...
  @ana
    Scenario: Search entities on the network with keywords
    Given I setup 4 nodes
    When I call publish on node 1 with validAssertion with keywords:
      | keyword 1 | keyword 2 |
    And I wait for last publish to finalize
    And Last publish finished with status: COMPLETED
    When I call search request on node 1 with result type entities for the keywords:
      | keyword 1 | keyword 2 |
    And Last search returned valid result
    Then The result of assertion search cannot be 0
    And The search result should contain all valid data
